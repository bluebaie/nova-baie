  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-[0_4px_24px_rgba(22,58,112,0.08)] transition-shadow duration-300 hover:shadow-[0_12px_40px_rgba(22,58,112,0.14)]">
      <div className="relative">
        ...
        <span
          className={`absolute left-3 top-3 z-10 rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-wider shadow-sm ${categoryStyles[item.categorie] ?? 'bg-nova-horizon text-nova-navy'}`}
        >
          {item.categorie}
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <h3 className="mt-2 text-base font-semibold leading-7 text-nova-navy">{item.titre}</h3>

        <div className="mt-3 flex-1 text-sm leading-7 text-nova-text">
          {expanded ? <span className="whitespace-pre-line">{item.contenuComplet}</span> : item.resume}
        </div>
