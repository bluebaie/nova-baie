...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: { title: t('metaTitle'), description: t('metaDescription'), url: `${base}/services` },
  };
}
...
export default async function ServicesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const offers = t.raw('offers') as Array<{ name: string; forWho: string; change: string }>;
  const diffItems = t.raw('diffItems') as string[];
      ...
      <section className="relative overflow-hidden bg-nova-navy pb-12 pt-32 md:pb-20">
        ...
        <Container>
          <div className="relative z-10 max-w-3xl">
            ...
            <p className="mt-5 leading-8 text-white/75">{t('description')}</p>
          </div>
        </Container>
      </section>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-1 flex-col">
                    <p className={`text-[11px] uppercase tracking-[0.22em] ${featured ? 'text-white/70' : 'text-nova-text/60'}`}>
                      {offer.name}
                    </p>
                    <p className={`mt-5 text-xs font-medium ${featured ? 'text-white/55' : 'text-nova-text/55'}`}>
                      {t('forWhoLabel')}
                    </p>
                    <p className={`mt-1 text-sm leading-7 ${featured ? 'text-white/90' : 'text-nova-text'}`}>
                      {offer.forWho}
                    </p>
                    <p className={`mt-5 text-xs font-medium ${featured ? 'text-white/55' : 'text-nova-text/55'}`}>
                      {t('changeLabel')}
                    </p>
                    <p className={`mt-1 flex-1 text-sm font-medium leading-7 ${featured ? 'text-white' : 'text-nova-navy'}`}>
                      {offer.change}
                    </p>
                    <div className="mt-8">
                      <Link
                        ...
                        className={[
                          ...
                          featured ? 'bg-white text-nova-navy shadow-sm hover:bg-white/92' : 'bg-nova-navy text-white hover:opacity-90',
                        ].join(' ')}
                      >
                        {t('cta')}
                      </Link>
        ...
        <Container>
          <div className="grid gap-10 md:grid-cols-2 md:items-center">
            <div>
              ...
              <h2 className="mt-5 text-nova-navy">{t('diffTitle')}</h2>
              <p className="mt-4 text-sm leading-8 text-nova-text">{t('diffDescription')}</p>
            </div>
      ...
      <section className="bg-nova-navy py-12 md:py-20">
        <Container>
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-white">{t('finalCtaTitle')}</h2>
            <div className="mt-8">
              <Link
                href="/contact"
